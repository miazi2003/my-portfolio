...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-4 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-7xl z-50 rounded-full transition-all duration-500 border ${
        scrolled
          ? "bg-black/80 backdrop-blur-xl border-white/40 py-3 shadow-[0_10px_30px_rgba(0,0,0,0.5)]"
          : "bg-zinc-950/20 backdrop-blur-md border-white/40 py-4"
      }`}
    >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-[calc(100%+8px)] left-0 right-0 md:hidden bg-zinc-900/95 backdrop-blur-2xl border border-white/10 rounded-2xl p-4 shadow-2xl"
          >
