...
const Banner = () => {
  ...
  return (
    <section
      id="home"
      className="relative min-h-screen flex items-center justify-center pt-36 lg:pt-20 px-6 overflow-hidden bg-black text-white"
    >
      {/* Background */}
    <Background></Background>
      ...
      <div className="max-w-6xl w-full grid lg:grid-cols-2 gap-12 items-center relative z-10">
        ...
        <motion.div
          ...
          transition={{ duration: 0.8 }}
          className="relative flex justify-center order-1 lg:order-2 will-change-transform transform-gpu"
        >
          <motion.div
            variants={floatingVariants}
            animate="animate"
            className="relative w-72 h-72 sm:w-[23rem] sm:h-[23rem] lg:w-[26rem] lg:h-[26rem] flex items-center justify-center will-change-transform transform-gpu"
          >
            <div className="relative z-10 w-full h-full rounded-full bg-zinc-900/40 border border-white/10 p-4 will-change-transform transform-gpu">
              <div className="w-full h-full rounded-full overflow-hidden border border-white/5 will-change-transform transform-gpu">
                <img
                  src={yeasinImg}
                  alt="Yeasin Miazi"
                  className="w-full h-full object-cover object-top hover:scale-110 transition-transform duration-700 will-change-transform transform-gpu"
                />
              </div>
            </div>

            <div className="absolute inset-0 rounded-full border border-white/10 scale-[1.12] opacity-40 will-change-transform transform-gpu" />
            <div className="absolute inset-0 rounded-full border border-white/5 scale-[1.28] opacity-25 will-change-transform transform-gpu" />

            {orbitingBadges.map((badge) => (
              <motion.div
                key={badge.label}
                className="absolute inset-0 flex items-center justify-center pointer-events-none will-change-transform transform-gpu"
                animate={{ rotate: 360 }}
                transition={{
              ...
              >
                <div className="relative w-[110%] h-[110%] will-change-transform transform-gpu">
                  <motion.div
                    className="
